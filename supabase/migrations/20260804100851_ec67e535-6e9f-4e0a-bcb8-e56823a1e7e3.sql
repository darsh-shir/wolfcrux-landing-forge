...
BEGIN
  ...
  -- STO stays positive-only; LTO now mirrors losses as a negative amount.
  IF v_net > 0 THEN
...
-- Recalculate all months so loss months get their negative LTO entries.
UPDATE trader_config SET updated_at = now() WHERE lto_percentage > 0;