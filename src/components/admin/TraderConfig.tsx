...
const TraderConfig = ({ users }: TraderConfigProps) => {
  ...
  // Recompute sto_ledger + lto_ledger for a given user/month/year using
  // the latest trader_config %. Keeps ledgers in sync with config edits.
  const syncLedgersForMonth = async (
    userId: string,
    month: number,
    year: number,
    stoPct: number,
    ...
    // Aggregate trading_data for that month (primary trader rows only — matches PayoutSheet/trigger logic)
    ...
    const { data: trades } = await supabase
      .from("trading_data")
      .select("net_pnl, shares_traded")
      .eq("user_id", userId)
      .gte("trade_date", start)
      .lt("trade_date", end);

    const grossProfit = (trades ?? []).reduce((s, t: any) => s + Number(t.net_pnl || 0), 0);
    const sharesTraded = (trades ?? []).reduce((s, t: any) => s + Number(t.shares_traded || 0), 0);
    const shareCost = (sharesTraded / 1000) * 14;
    const netProfit = grossProfit - shareCost - Number(softwareCost || 0);

    const stoAmount = netProfit > 0 ? netProfit * (Number(stoPct) / 100) : 0;
    const ltoAmount = netProfit > 0 ? netProfit * (Number(ltoPct) / 100) : 0;

    // ---------- STO ledger ----------
    const { data: existingSto } = await supabase
      .from("sto_ledger")
      .select("id, leave_deduction_amount, trainee_pool_contribution")
      .eq("user_id", userId).eq("month", month).eq("year", year)
      .maybeSingle();

    if (existingSto?.id) {
      const leaveDed = Number(existingSto.leave_deduction_amount || 0);
      const traineePool = Number(existingSto.trainee_pool_contribution || 0);
      await supabase.from("sto_ledger").update({
        gross_profit: grossProfit,
        shares_traded: sharesTraded,
        share_cost: shareCost,
        software_cost: softwareCost,
        net_profit: netProfit,
        sto_percentage: stoPct,
        sto_amount: stoAmount,
        final_sto_amount: stoAmount - traineePool - leaveDed,
      }).eq("id", existingSto.id);
    }

    // ---------- LTO ledger ----------
    const { data: existingLto } = await supabase
      .from("lto_ledger")
      .select("id, is_released")
      .eq("user_id", userId).eq("month", month).eq("year", year)
      .maybeSingle();

    if (ltoPct > 0 && ltoAmount > 0) {
      if (existingLto?.id) {
        await supabase.from("lto_ledger").update({
          net_profit: netProfit,
          lto_percentage: ltoPct,
          lto_amount: ltoAmount,
        }).eq("id", existingLto.id);
      } else {
        await supabase.from("lto_ledger").insert({
          user_id: userId,
          month, year,
          net_profit: netProfit,
          lto_percentage: ltoPct,
          lto_amount: ltoAmount,
          unlock_date: `${year + 1}-${String(month).padStart(2, "0")}-01`,
        });
      }
    } else if (existingLto?.id && !existingLto.is_released) {
      // LTO turned off / zero — drop unreleased row
      await supabase.from("lto_ledger").delete().eq("id", existingLto.id);
    }
  };
